...
function update_menu($dbc, $menu_data) {
    ...
    try {
        $q = "UPDATE `brit_menu` SET `menu_title`= :title, `menu_anchor`= :anchor, `menu_parent_id`= :parent,`menu_sort`= :sort, `menu_admin` = :admin WHERE `menu_id` = $menu_data[id]"; //debug_r($q); die();
        if ($sql = $dbc->prepare($q)) {
            ...
            $sql->bindParam(':admin', $menu_data['admin']);
            return $sql->execute();
        }
